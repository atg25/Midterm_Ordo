...
type SeriesPoint = {
  xValue: Exclude<GraphValue, null | undefined>;
  yValue: number;
  seriesKey: string;
};
...
function getCategoricalDomain(
  ...
  const values = rows
    .map((row) => row[field])
    .filter(
      (value): value is GraphValue => value !== undefined && value !== null,
    );
  ...
  if (type === "quantitative" || type === "temporal") {
    const sortable = values
      ...
      .filter(
        (entry): entry is { value: GraphValue; numeric: number } =>
    ...
    return sortable
      .filter(
        (entry, index, list) =>
          list.findIndex((candidate) => candidate.numeric === entry.numeric) ===
          index,
      )
...
function buildCsv(columns: string[], rows: GraphRow[]): string {
  ...
  const body = rows
    .map((row) =>
      columns.map((column) => escapeCell(row[column] ?? null)).join(","),
    )
    .join("\n");
  return `${header}\n${body}`;
}
...
function getGraphValidationIssue(graph: GraphSpec): string | undefined {
  switch (graph.kind) {
    ...
    case "histogram":
      return graph.x
        ? undefined
        : "Histogram graphs require an x encoding before they can render.";
    case "heatmap":
      if (!graph.x || !graph.y) {
  ...
  return graph.data
    .map((row) => {
      ...
      if (xValue === undefined || xValue === null || numericY === undefined)
        return null;
      const validXValue = xValue as Exclude<GraphValue, null | undefined>;
      return {
        xValue: validXValue,
        yValue: numericY,
        seriesKey: series
          ? String(row[series.field] ?? "Unspecified")
          : "Series 1",
      };
    })
...
export function GraphRenderer({
  ...
  const headerSubtitle =
    title?.trim() && caption?.trim()
      ? caption.trim()
      : `${inferredKind} · Studio graph`;
  const exportStem = toFileStem(
    downloadFileName || title || caption || inferredKind,
  );
    ...
    <>
      ...
      {previewRows.length > 0 ? (
        <button
          ...
          className="focus-ring flex h-9 min-w-9 items-center justify-center rounded-md text-foreground opacity-50 transition-all hover:opacity-100 hover-surface active:scale-90 sm:h-8 sm:min-w-8"
          aria-label={
            isPreviewVisible ? "Hide data preview" : "Show data preview"
          }
  ...
  const handleDownload = useCallback(() => {
    ...
    downloadBlob(
      ...
      `${exportStem}_${timestamp}.svg`,
    );
  }, [graph.kind, columns, graph.data, exportStem]);
    ...
    return (
      ...
      >
        <div className="w-full overflow-x-auto p-3">
          {sourceLabel ? (
            <p
              className="px-1 pb-2 text-[11px] font-semibold uppercase tracking-[0.16em] text-foreground/52"
          ...
          {hasRows ? (
            ...
            <div
              className="flex min-h-28 items-center justify-center text-xs opacity-60"
  ...
  const isBarLike =
    graph.kind === "bar" ||
    graph.kind === "grouped-bar" ||
    graph.kind === "stacked-bar" ||
    graph.kind === "histogram";
  const isXYPlot =
    graph.kind === "line" ||
    graph.kind === "area" ||
    graph.kind === "scatter" ||
  ...
  const useContinuousX =
    isXYPlot &&
    (graph.x?.type === "quantitative" || graph.x?.type === "temporal");
  const categoricalX = !useContinuousX;
  const categoricalDomain = graph.x
    ? getCategoricalDomain(graph.data, graph.x.field, graph.x.type)
    : [];
  ...
  const categoricalXScale = (value: GraphValue) => {
    const index = Math.max(
      categoricalDomain.findIndex((entry) => entry === value),
      0,
    );
    if (isBarLike) {
    ...
    return (
      margin.left +
      (index / Math.max(categoricalDomain.length - 1, 1)) * innerWidth
    );
  };
  const continuousXScale = (value: number) =>
  ...
  const barGroups =
    graph.kind === "bar" || graph.kind === "grouped-bar"
      ? categoricalDomain.map((domainValue) => {
          return seriesKeys.map((seriesKey) => ({
            ...
            total: points
              .filter(
                (point) =>
                  point.xValue === domainValue && point.seriesKey === seriesKey,
              )
              .reduce((sum, point) => sum + point.yValue, 0),
          }));
        })
      : [];
  const stackedBarGroups =
    graph.kind === "stacked-bar"
      ? categoricalDomain.map((domainValue) => ({
          domainValue,
  ...
  const heatmapXDomain =
    graph.kind === "heatmap" && graph.x
      ? getCategoricalDomain(graph.data, graph.x.field, graph.x.type)
      : [];
  const heatmapYDomain =
    graph.kind === "heatmap" && graph.y
      ? getCategoricalDomain(graph.data, graph.y.field, graph.y.type)
      : [];
  const heatmapColorValues =
    graph.kind === "heatmap" && graph.color
      ? (() => {
          const colorField = graph.color.field;
          return graph.data
            .map((row) => Number(row[colorField] ?? 0))
  ...
  const xTickValues = categoricalX
    ? categoricalDomain.filter(
        (_, index, list) =>
          list.length <= 6 || index % Math.ceil(list.length / 6) === 0,
      )
      ...
      <div className="w-full p-3">
        {validationIssue ? (
          ...
          <div className="space-y-3">
            {sourceLabel ? (
              <p
                className="px-2 text-[11px] font-semibold uppercase tracking-[0.16em] text-foreground/52"
                data-testid="graph-source"
              >
                Source: {sourceLabel}
              </p>
            ...
            {graph.series ? (
              <div
                className="flex flex-wrap gap-3 px-2 text-[11px] uppercase tracking-wider text-foreground/65"
                ...
                {seriesKeys.map((seriesKey) => (
                  ...
                  >
                    <span
                      ...
                      style={{
                        backgroundColor: getSeriesColor(seriesKeys, seriesKey),
                      }}
                    />
                    {seriesKey}
                  </span>
            ...
            {summary ? (
              <p
                className="px-2 text-sm leading-6 text-foreground/78"
                ...
                >
                  ...
                  {categoricalX
                    ? (xTickValues as GraphValue[]).map((tick) => {
                        const x = isBarLike
                          ? categoricalXScale(tick) +
                            innerWidth /
                              Math.max(categoricalDomain.length, 1) /
                              2
                          : categoricalXScale(tick);
                        return (
                          <g key={`x-${String(tick)}`}>
                    ...
                    : (xTickValues as number[]).map((tick) => (
                        <g key={`x-${tick}`}>
                          <line
                            x1={continuousXScale(tick)}
                            x2={continuousXScale(tick)}
                            y1={margin.top + innerHeight}
                            y2={margin.top + innerHeight + 6}
                            stroke="currentColor"
                            strokeOpacity="0.4"
                    ...
                    ? groupedPoints.map((group) => {
                        const orderedPoints = group.points
                          ...
                          .sort((left, right) => {
                            if (useContinuousX) {
                              const leftValue =
                                toNumber(left.xValue, graph.x?.type) ?? 0;
                              const rightValue =
                                toNumber(right.xValue, graph.x?.type) ?? 0;
                              return leftValue - rightValue;
                            }
                            const leftIndex = categoricalDomain.findIndex(
                              (entry) => entry === left.xValue,
                            );
                            const rightIndex = categoricalDomain.findIndex(
                              (entry) => entry === right.xValue,
                            );
                        ...
                        const path = orderedPoints
                          .map((point, index) => {
                            const x = useContinuousX
                              ? continuousXScale(
                                  toNumber(point.xValue, graph.x?.type) ?? 0,
                                )
                        ...
                        return (
                          <g key={group.key}>
                              ...
                              <path
                                d={`${path} L ${
                                  ...
                                  useContinuousX
                                    ? continuousXScale(
                                        toNumber(
                                          orderedPoints[0]?.xValue ?? 0,
                                          graph.x?.type,
                                        ) ?? 0,
                                      )
                                    : categoricalXScale(
                                        orderedPoints[0]?.xValue ?? null,
                                      )
                                } ${yScale(0)} Z`}
                            ...
                            <path
                              d={path}
                              fill="none"
                              stroke={getSeriesColor(seriesKeys, group.key)}
                              strokeWidth="3"
                              strokeLinejoin="round"
                              strokeLinecap="round"
                              data-series-path={group.key}
                            />
                            {orderedPoints.map((point, index) => (
                              <circle
                                ...
                                cx={
                                  useContinuousX
                                    ? continuousXScale(
                                        toNumber(point.xValue, graph.x?.type) ??
                                          0,
                                      )
                                    : categoricalXScale(point.xValue)
                                }
                  ...
                  graph.kind === "histogram"
                    ? barGroups.map((group, groupIndex) => {
                        const band =
                          innerWidth / Math.max(categoricalDomain.length, 1);
                        ...
                        const barWidth =
                          availableWidth / Math.max(seriesKeys.length, 1);
                  ...
                  {graph.kind === "stacked-bar"
                    ? stackedBarGroups.map((group, groupIndex) => {
                        ...
                        return group.series.map((entry) => {
                          const x =
                            margin.left + groupIndex * band + groupPadding;
                    ...
                    ? groupedPoints.map((group) => (
                        <g key={group.key}>
                          {group.points.map((point, index) => {
                            const numericX = toNumber(
                              point.xValue,
                              graph.x?.type,
                            );
                            const x =
                              numericX !== undefined && !categoricalX
                                ? continuousXScale(numericX)
                                : categoricalXScale(point.xValue);
                            const radius =
                              graph.kind === "bubble" && sizeField
                                ? bubbleRadius(
                                    graph.data.find((row) => {
                                      const xValue = row[graph.x?.field ?? ""];
                                      const yValue = row[graph.y?.field ?? ""];
                                      const seriesValue = graph.series
                                        ? String(
                                            row[graph.series.field] ??
                                              "Unspecified",
                                          )
                                        : "Series 1";
                                      return (
                  ...
                  {graph.kind === "heatmap" && graph.x && graph.y && graph.color
                    ? (() => {
                        ...
                        const colorField = graph.color.field;
                        return graph.data.map((row, index) => {
                          const xIndex = heatmapXDomain.findIndex(
                            (entry) => entry === row[xField],
                          );
                          const yIndex = heatmapYDomain.findIndex(
                            (entry) => entry === row[yField],
                          );
                          const cellWidth =
                            innerWidth / Math.max(heatmapXDomain.length, 1);
                          const cellHeight =
                            innerHeight / Math.max(heatmapYDomain.length, 1);
                          const x = margin.left + xIndex * cellWidth;
                          const y = margin.top + yIndex * cellHeight;
                          const value = Number(row[colorField] ?? 0);
                          return (
                            <rect
                              key={`heat-${index}`}
                              x={x}
            ...
            {previewRows.length > 0 && isPreviewVisible ? (
              <div
                className="overflow-x-auto rounded-theme border-theme bg-surface-muted/45 p-2"
                data-testid="graph-data-preview"
              >
                    ...
                    {previewRows.map((row, rowIndex) => (
                      ...
                      >
                        {previewColumns.map((column) => (
                          <td
                            key={`${rowIndex}-${column}`}
                            className="border-t border-border/60 px-3 py-2 align-top text-foreground/78"
                          >
                            {formatValue(
                              row[column] ?? null,
                              column === graph.x?.field
                                ? graph.x?.type
                                : column === graph.y?.field
                                  ? graph.y?.type
                                  : undefined,
                            )}
                          </td>
          ...
          <div
            className="flex min-h-32 items-center justify-center text-xs opacity-60"
            data-testid="graph-empty-state"
          >
            No data available yet.
          </div>
